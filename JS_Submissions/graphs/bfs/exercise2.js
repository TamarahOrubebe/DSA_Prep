...
var snakesAndLadders = function(board) {
    const getLabelCoordinates = (label, n) => {
        let row = Math.floor((label - 1) / n);
        let col = row % 2 == 0? (label - 1) % n :  n - 1 - (label - 1) % n;
    ...
    while(queue.length) {
       const currentLength = queue.length;
        ...
        for(let i = 0; i < currentLength; i++) {
            ...
            if(label == n*n) return moves;
            if(visited.has(label)) continue;
            visited.add(label);
            for(let next = label + 1; next <= Math.min(label + 6, n*n ); next++) {
                let [row, col] = getLabelCoordinates(next, n);
                let destination = board[row][col] == -1? next : board[row][col];

...
You are also given an integer truckSize, which is the maximum number of boxes that can be put on the truck. You 
...
var maximumUnits = function(boxTypes, truckSize) {
    ...
    while (i < boxTypes.length) {
      sum += boxTypes[i][0];
        if (sum > truckSize) {
            sum -= boxTypes[i][0];
            let extra = truckSize - sum;
            sum += extra;
            ans += extra * boxTypes[i][1];
            break;
        } else {
